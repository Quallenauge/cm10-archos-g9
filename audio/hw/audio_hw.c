...
#define MIXER_HS_HIGH_POWER_MODE            "High-Performance"
...
// #define MIXER_450HZ_BAND_PASS               "Band-pass 450-14500Hz"
...
#ifdef OUT_SAMPLING_RATE
#define DEFAULT_OUT_SAMPLING_RATE OUT_SAMPLING_RATE
#else
...
#define PRODUCT_DEVICE_ARCHOS_80     "A80"
#define PRODUCT_DEVICE_ARCHOS_101   "A101"
...
static void printMultimediaMixerState(struct omap_audio_device *adev)
{
struct mixer_ctl *mm_dl2 = mixer_get_ctl_by_name(adev->mixer, MIXER_DL2_MIXER_MULTIMEDIA);
int state = mixer_ctl_get_value(mm_dl2, 0);
ALOGV(">>>>>>>>>>>>>>>>Multimedia Mixer state: %d.\n", state);
}
...
static int get_boardtype(struct omap_audio_device *adev)
{
    ...
    if(!strcmp(board, PRODUCT_DEVICE_ARCHOS_80) || !strcmp(board, PRODUCT_DEVICE_ARCHOS_101)) {
            adev->board_type = OMAP4_ARCHOS;
          /*true on devices that must use sidetone capture */
            adev->sidetone_capture = 1;
    }
    else{
      ALOGI("Invalid board: %s !", board);
      return -EINVAL;
    }
                              ...
                              int headset_mic_on, int sub_mic_on)
{
    ...
    LOGFUNC("%s(%p, %d, %d, %d)", __FUNCTION__, adev, main_mic_on, headset_mic_on, sub_mic_on);
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
	    ...
	    LOGFUNC("%s(%p, %d, %d, %d) AUDIO_SOURCE_MIC", __FUNCTION__, adev, main_mic_on, headset_mic_on, sub_mic_on);
	    ...
	    LOGFUNC("%s(%p, %d, %d, %d) AUDIO_SOURCE_CAMCORDER", __FUNCTION__, adev, main_mic_on, headset_mic_on, sub_mic_on);
	    ...
	    LOGFUNC("%s(%p, %d, %d, %d) AUDIO_SOURCE_VOICE_RECOGNITION", __FUNCTION__, adev, main_mic_on, headset_mic_on, sub_mic_on);
	    ...
	    LOGFUNC("%s(%p, %d, %d, %d) AUDIO_SOURCE_VOICE_COMMUNICATION", __FUNCTION__, adev, main_mic_on, headset_mic_on, sub_mic_on);
    ...
    LOGFUNC("%s(%p, %d, %d, %d) Setting volume to %d.", __FUNCTION__, adev, main_mic_on, headset_mic_on, sub_mic_on, volume);
...
static void force_all_standby(struct omap_audio_device *adev)
{
    ...
    printMultimediaMixerState(adev);
}
    ...
    struct omap_stream_out *out)
{
    ...
    LOGFUNC("%s:%d(%p) adev->dl2_support=%d", __FUNCTION__, __LINE__, adev, adev->dl2_support);
    printMultimediaMixerState(adev);
    ...
    if (out) {
      ...
      LOGFUNC("%s:%d(%p): out parameter can is null", __FUNCTION__, __LINE__, adev);  
    }
    
    LOGFUNC("%s:%d(%p)", __FUNCTION__, __LINE__, adev);
    printMultimediaMixerState(adev);
    ...
    if (out) {
        if (mm_stream) {
            ...
            if (adev->dl2_support){
	     if (adev->board_type == OMAP4_ARCHOS){
	       ALOGV("Setting mixer_ctls.mm_dl2 to value: %d", set_speaker_on);
	       mixer_ctl_set_value(adev->mixer_ctls.mm_dl2, 0, set_speaker_on);
	     }else{
    ...
    LOGFUNC("%s:%d(%p): set_speaker_on=%d set_dl1_on=%d", __FUNCTION__, __LINE__, adev,set_speaker_on, set_dl1_on);
    printMultimediaMixerState(adev);
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        if (bt_on)
	      ...
	      LOGFUNC("%s:%d: headset_on || headphone_on || earpiece_on", __FUNCTION__, __LINE__);
                set_route_by_array(
                        adev->mixer,
	      ...
	      LOGFUNC("%s:%d: speaker_on", __FUNCTION__, __LINE__);
                if ((adev->board_type == OMAP4_BLAZE) || adev->board_type == OMAP4_ARCHOS || 
                        (adev->board_type == OMAP5_SEVM)) {
                    set_route_by_array(
                            adev->mixer,
	        ...
	        LOGFUNC("%s:%d: else...", __FUNCTION__, __LINE__);
                if ((adev->board_type == OMAP4_BLAZE)  || adev->board_type == OMAP4_ARCHOS || 
                        (adev->board_type == OMAP5_SEVM)) {
	   ...
	   (adev->board_type == OMAP5_PANDA) ||
           (adev->board_type == OMAP5_SEVM)) {
            ...
            mixer_ctl_set_enum_by_string(adev->mixer_ctls.left_capture,
                    main_mic_on ? "off" :
                    (headset_on ? MIXER_HS_MIC : "Off"));
        }else if(adev->board_type == OMAP4_ARCHOS){
	    /* Select front end */
            if (main_mic_on || headset_on)
                set_route_by_array(adev->mixer, mm_ul2_amic_left, 1);
            else if (sub_mic_on)
            ...
            mixer_ctl_set_enum_by_string(adev->mixer_ctls.right_capture,
                    sub_mic_on ? MIXER_SUB_MIC : "Off");
	    // Archos: Always enable main mic - no headset mic supported
            mixer_ctl_set_enum_by_string(adev->mixer_ctls.left_capture,
                    MIXER_MAIN_MIC);
	}
    }
...
static int start_output_stream(struct omap_stream_out *out)
{
	...
	ALOGV("%s:%d: Using MM_FULL_POWER_SAMPLING_RATE",__FUNCTION__,__LINE__);
    }
    else{
        out->config.rate = DEFAULT_OUT_SAMPLING_RATE;
	ALOGV("%s:%d: Using DEFAULT_OUT_SAMPLING_RATE (44100)",__FUNCTION__,__LINE__);
    }
    ...
    if (fm_enable) {
      ALOGV("%s:%d: FM Enabled",__FUNCTION__,__LINE__);
                     ...
                     hw_device_t** device)
{
    ...
    if (adev->dl2_support) {
		ALOGE("Set route by array hf_dl2.");
        set_route_by_array(adev->mixer, hf_dl2, 1);
    } else {
		ALOGE("Set route by array hf_dl1.");
